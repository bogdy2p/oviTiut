...
while($i <= $file_list_count-1) {
	...
	if($filename == "Output" && $TaskName == "Phasing") {
...
$url = $this->container->getParameter('apiUrl')."campaigns/".$project_id."/weeklyphasing";
...
<div class="container-fluid min-height">
	<div class="row" style="display: flex;">
			...
			<div class="col-xs-offset-10 col-xs-offset-1 min-height campaign-content-container">
				...
				<div class="row get_api" data-api="<?php echo $_COOKIE['api']; ?>">
					<div class="col-xs-12">
						...
						<h1 class="evo-header-big margin-top-0 text-swap font_stack_4">Phasing</h1>
						<div class="evo-space"></div>
					</div>

					<div class="col-xs-12">
						<div class="task_bar">
							<div class="task_bar_left project_id" data-project-id="<?php echo $project_id; ?>">								
								<a href="<?php echo $matrixLink; ?>&screen=10100"><button class="btn evo-btn-2 await_response3">Launch Phasing</button></a>
							</div>
					...
					<div class="col-xs-11">
						<div id="chart-container" style="width:660px; height:440px; opacity: 0; display: none;"></div>						<div class="placeholder fluid-img project_id" data-project-id="<?php echo $project_id; ?>">
							<?php if($has_data) { ?><script>$(document).ready(function() { render_phasing(); });</script><?php } else { ?><img src="http://placehold.it/900x400&text=chart"><?php } ?>
						</div>
					</div>
									...
									<div class="file_info_container">
										<div class="file_task_icon">
											...
											<div class="box_file_tout">
												<ul>
													<?php if($userCanEdit) { ?>
														<li><a href="#" class="replace_file_1" data-text=" attach phasing output">Replace File</a></li>
														<li>
													<?php } ?>
														<a href="<?php echo $view['router']->generate('initiative_app_download_file'); ?>?task_name_id=7&file_type_id=11&campaign_id=<?php echo $project_id; ?>">
															Download File
														</a>
													</li>
												</ul>
					...
					<div class="col-xs-12">
						...
						<div class="row" style="display: flex;">
							<div class="col-xs-12" style="display: flex;">
								<div class="file_box file_1<?php if(!isset($data1) && $userCanEdit){echo " file_drop_1";}?>" data-form-id="1" data-project-id="<?php echo $project_id; ?>" data-task-id="<?php echo $real_TaskID; ?>" data-task-type="11" data-file-name="<?php echo $project_id; ?>_Phasing.png">
									<?php if(isset($data1)) { ?>
										<a href="<?php echo $view['router']->generate('initiative_app_download_file'); ?>?task_name_id=7&file_type_id=11&campaign_id=<?php echo $project_id; ?>">
											<i class="fa fa-cloud-download"></i> 
											download phasing output  
										</a>
									<?php } else {	?>
										<i class="fa fa-file-o"></i> <?php echo ($userCanEdit) ? 'attach phasing output' : 'phasing output unavailable'; ?> 
									<?php }
									?>
								</div>
							</div>
		...
		<div class="col-xs-4 sidebar_bg">
			<?php echo $view->render('InitiativeAppBundle:Inc:CampaignSidebar/index.html.php', array('campaignName' => $campaign['Campaign']['CampaignName'], 'userCanEdit' => $userCanEdit, 'project_id' => $project_id, 'task_id' => "700", 'real_TaskID' => $real_TaskID)); ?>
		</div>
	</div>
</div>